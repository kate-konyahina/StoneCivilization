...
public class WorkingPlaceMountains : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "NPC")
        {
            ...
            NPC.GetComponent<CharacterInfo>().atWorkingPlaceMountains = true;
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "NPC")
        {
            isFree = true;
            var NPC = collision.gameObject;
            NPC.GetComponent<CharacterInfo>().atWorkingPlaceMountains = false;
        }
    }
}
